...
export class TemReportComponent{
    private webFilePath: string = "";//经过编码的文件全路径(eg,d:/XX/XX.html)
    private reportUrl: string = ""; //虚拟目录请求路径(eg,http://ip:8080/report/templateFile)
    private webPath: string = "";   //虚拟目录实际路径(eg,d:/report/file/web/template)
        ...
        route.params.subscribe(
            param=>{
                ...
                if(this.reportUrl==""&&this.webPath==""){
                    this.http.post(this.appCommonService.getReportServerPath()+"/dynamicReport/MyTemplateUrl",null,{headers: this.appCommonService.getHeaders()}).toPromise().then(
                        res=>{ 
                            this.reportUrl = this.appCommonService.getDynamicReportPath();
